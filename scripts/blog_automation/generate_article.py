...
UNSUPPORTED TECHNICAL SPECULATION IS PROHIBITED: never guess what the product likely/probably/presumably/might use, include, support, contain, require, run, or how it may be trained, fine-tuned, architected, hosted, or deployed. Do not infer capabilities from a feature or product name. Do not list hypothetical APIs, interfaces, plugins, protocols, or architectures and then disclaim that the details were not disclosed. Do not predict that a product will improve security, reliability, performance, or outcomes unless that result is explicitly measured or confirmed by an approved source. Do not fill gaps with hedged claims such as 'likely includes', 'probably uses', 'probable specialized fine-tuning', 'presumably', or 'might include'. For anything the official sources do not disclose, say explicitly that it was not disclosed, without proposing imagined alternatives. Technical analysis may discuss only consequences directly derivable from confirmed behavior, comparison to public documentation, and practical developer impact. A clearly labeled possibility about workflow impact (for example, what a developer could test or choose to do) is acceptable; a possibility attributed to undocumented product capabilities is not.
